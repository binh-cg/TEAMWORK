...
# 2. API Đăng ký thông tin và Mã hóa mật khẩu đầu ra
...
# 4. API Lấy danh sách chuyến bay
...
def api_chuyen_bay():
    ...
    for cb in chuyen_bays:
        result.append({
            ...
            "ngayGioDi": cb.ngayGioDi.strftime("%Y-%m-%d %H:%M"),
            "ngayGioDen": cb.ngayGioDen.strftime("%Y-%m-%d %H:%M"),
            "giaVe": cb.giaVe,
            "soGheTrong": cb.soGheTrong
        })
    return jsonify({"status": "success", "chuyenBays": result}), 200

# 5. API Tìm kiếm chuyến bay 
...
def api_chuyen_bay_search():
    ...
    if san_bay_di:
        query = query.filter(ChuyenBay.sanBayDi.like(f'%{san_bay_di}%'))
    if san_bay_den:
        query = query.filter(ChuyenBay.sanBayDen.like(f'%{san_bay_den}%'))

    chuyen_bays = query.all()
    result = []
    for cb in chuyen_bays:
        if ngay_gio_di and cb.ngayGioDi.strftime("%Y-%m-%d") != ngay_gio_di:
            continue
        result.append({
            "maChuyenBay": cb.maChuyenBay,
            "maMayBay": cb.maMayBay,
...
# 6. API Đặt vé và tự động nạp hành khách đi kèm (ĐÃ FIX LỖI)
...
def api_dat_ve():
    ...
    if not user or not chuyen_bay:
        return jsonify({"status": "error", "message": "Người dùng hoặc chuyến bay không tồn tại!"}), 404

    so_luong_mua = len(danh_sach_hanh_khach) if danh_sach_hanh_khach else 1
    if chuyen_bay.soGheTrong < so_luong_mua:
        return jsonify({"status": "error", "message": f"Chuyến bay không đủ chỗ! Chỉ còn {chuyen_bay.soGheTrong} ghế trống."}), 400

    tong_tien = chuyen_bay.giaVe * so_luong_mua
    ma_dat_ve = ''.join(random.choices(string.ascii_uppercase + string.digits, k=6))

    try:
        phieu_dat_ve = PhieuDatVe(
            maDatVe=ma_dat_ve,
            tongTien=tong_tien,
            phuongThucThanhToan=phuong_thuc_thanh_toan,
            user_id=user_id,
            chuyenbay_id=chuyenbay_id
        )
        db.session.add(phieu_dat_ve)
        
        for hk in danh_sach_hanh_khach:
            hanh_khach = HanhKhach(
                id='HK' + ''.join(random.choices(string.digits, k=4)),
                hoTen=hk.get('hoTen'),
                cccd=hk.get('cccd'),
                soDienThoai=hk.get('soDienThoai'),
                phieu_id=ma_dat_ve
            )
            db.session.add(hanh_khach)
            
        # Tự động trừ số ghế trống hệ thống
...
# 7. API Xác Nhận Thanh Toán Vé
@app.route('/api/ve/thanh-toan/<ma_ve>', methods=['PUT'])
def api_thanh_toan_ve(ma_ve):
    phieu = PhieuDatVe.query.get(ma_ve)
    if not phieu:
        return jsonify({"status": "error", "message": "Không tìm thấy mã vé này"}), 404
        
    if phieu.trangThaiDatVe != 'Chưa Thanh Toán':
        return jsonify({"status": "error", "message": f"Vé này đang ở trạng thái: {phieu.trangThaiDatVe}"}), 400
...
def api_huy_ve(ma_ve):
    phieu = PhieuDatVe.query.get(ma_ve)
    if not phieu:
        return jsonify({"status": "error", "message": "Không tìm thấy mã vé"}), 404
        
    if phieu.trangThaiDatVe == 'Đã Hủy':
        return jsonify({"status": "error", "message": "Vé này đã được hủy trước đó rồi"}), 400
    ...
    db.session.commit()
    return jsonify({"status": "success", "message": f"Hủy vé thành công. Hệ thống đã hoàn trả lại {so_luong_hoan_ghe} ghế trống."}), 200

#9.API Giao diện đặt vé
# Route giao diện điền thông tin đặt vé
...
def web_dat_ve():
    ...
    if not chuyen_bay:
        flash('Chuyến bay không tồn tại hoặc đã bị hủy!', 'danger')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Lưu thông tin hành khách đi kèm
            hanh_khach = HanhKhach(
                id='HK' + ''.join(random.choices(string.digits, k=4)),
                hoTen=ho_ten,
                cccd=cccd,
            ...
            # Trừ bớt 1 ghế trống của chuyến bay
...
#------- KHỞI TẠO HỆ THỐNG VÀ CHẠY SERVER -------

def tao_du_lieu():
    if User.query.first():
        return 'Dữ liệu mẫu đã có sẵn.'
    
    db.session.add(User(id='U001', taiKhoan='admin', matKhau=generate_password_hash('admin123'), hoTen='Admin', email='admin@example.com', role='admin'))
    db.session.add(User(id='U002', taiKhoan='nhanvien', matKhau=generate_password_hash('nv123'), hoTen='Nhân Viên', email='nhanvien@example.com', role='nhanvien'))
    db.session.add(User(id='U003', taiKhoan='khachhang', matKhau=generate_password_hash('kh123'), hoTen='Khách Hàng', email='khachhang@example.com', role='khachhang'))
